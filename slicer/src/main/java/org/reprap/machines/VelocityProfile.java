 ...
 * 
 * The result is either a single maximum velocity, v, in between the ends (in
 * which case flat will be 1), or an acceleration to maxSpeed at s1 from the
 * start, movement at that velocity to s2, then deceleration to the end (in
 * which case flat will be 2).
 * 
 * If flat is 0 on return, the starting and ending speeds are greater than the
 * maximum allowed...
 * 
 * @author Adrian
 * 
...
public class VelocityProfile {
    ...
    public VelocityProfile(final double s, final double vStart, final double maxSpeed, final double vEnd,
